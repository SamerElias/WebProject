...
/**
 * Servlet purchase check on book.
 */
@WebServlet(urlPatterns = { "/GetOwnedStatus" }, 
		initParams = { 
				@WebInitParam(name = "bookName", value = "")
		})
public class GetOwnedStatus extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetOwnedStatus() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(stmt != null) {stmt.close();}
			if(conn != null) {conn.close();}
			String customerJsonResult = gson.toJson(owned);
